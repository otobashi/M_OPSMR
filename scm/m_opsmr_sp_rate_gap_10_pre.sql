ALTER PROCEDURE [dbo].[m_opsmr_sp_rate_gap_10_pre]
(
...
/*************************************************************************
1. 프 로 젝 트 : M_OPSMR
2. 프로그램 ID : m_opsmr_sp_rate_gap_10_pre
3. 기     능 : DB2 기준가동률 및 운영가동률을 m_opsmr_sp_rate_gap_10_pre

--   EXEC m_opsmr_sp_rate_gap_10_pre 'STD', '201603', 85, 10
...
SELECT a.sub
      ...
      ,a.qty_rate_mon13
      ,a.qty_rate_1_base
      ,CASE WHEN a.pre_rate_gap >= 0 THEN 1
...
FROM  ( SELECT sub.display_name AS sub
              ...
              ,isnull(sum(case when a.kpi_period_code = @vc_pre_yyyymm then a.standard_operation_rate end),0) as pre_rate
              ,isnull(sum(case when a.kpi_period_code = @base_yyyymm then a.standard_operation_rate end),0) as cur_rate
              ,isnull(sum(case when a.kpi_period_code = @vc_post_1 then a.standard_operation_rate end),0) as post_rate
              ,(isnull(sum(case when a.kpi_period_code = @base_yyyymm then a.standard_operation_rate end),0)) - (isnull(sum(case when a.kpi_period_code = @vc_pre_yyyymm then a.standard_operation_rate end),0)) AS pre_rate_gap
              ,(isnull(sum(case when a.kpi_period_code = @vc_post_1 then a.standard_operation_rate end),0)) - (isnull(sum(case when a.kpi_period_code = @base_yyyymm then a.standard_operation_rate end),0)) AS post_rate_gap
              ,abs((isnull(sum(case when a.kpi_period_code = @base_yyyymm then a.standard_operation_rate end),0)) - (isnull(sum(case when a.kpi_period_code = @vc_pre_yyyymm then a.standard_operation_rate end),0))) AS abs_pre_rate_gap
              ,abs((isnull(sum(case when a.kpi_period_code = @vc_post_1 then a.standard_operation_rate end),0)) - (isnull(sum(case when a.kpi_period_code = @base_yyyymm then a.standard_operation_rate end),0))) AS abs_post_rate_gap
              ...
              ,case when isnull(sum(case when a.kpi_period_code between @vc_pre_13 and @base_yyyymm then a.production_capa end),0) = 0 then 0
                    else isnull(sum(case when a.kpi_period_code between @vc_pre_13 and @base_yyyymm then a.production_quantity end),0) / isnull(sum(case when a.kpi_period_code between @vc_pre_13 and @base_yyyymm then a.production_capa end),0) end AS rate_mon13
        ...
        AND    sub.sheet08_yn = 'Y'
        AND    prod.sheet08_yn = 'Y'
        AND    a.factory_region1 <> 'CROSS(KR)'
        AND    a.gbu_code = qty.gbu_code
        GROUP BY sub.display_name
                ,prod.display_name
      ) a
WHERE  a.cur_rate < @base_rate*0.01
AND    a.abs_pre_rate_gap > @chg_rate*0.01
;
