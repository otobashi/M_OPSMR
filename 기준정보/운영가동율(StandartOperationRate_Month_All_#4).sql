...
( 
   ...
   SELECT LV.FACTORY_REGION AS FACTORY_REGION1
    ...
    WHERE LV.LOOKUP_TYPE = 'PD_INPUT_TEMPLET_MANUAL_LINE'
      AND LV.ATTRIBUTE2 = :KPI_PERIOD_STD
...
SELECT MONTH_rn
     ...
     , CASE WHEN VALUE(PRODUCTION_CAPA,0) = 0 THEN 0
            ELSE PRODUCTION_QUANTITY / PRODUCTION_CAPA
	 ...
	 --20150116 고상현 : 임시 저장된 데이터를 보여주지 않기 위해 수정 
	 --PRODUCTION_QUANTITY가 0으로 판단하는 이유는 아래 서브쿼리에서 임시 저장된 수량을 제외한 수치이기 때문
     , VALUE(CASE WHEN VALUE(PRODUCTION_QUANTITY,0) = 0 THEN 0 ELSE LINE_COUNT_USE END,0) AS LINE_COUNT_USE 
     , VALUE(LINE_COUNT_TOTAL,0)-VALUE(CASE WHEN VALUE(PRODUCTION_QUANTITY,0) = 0 THEN 0 ELSE LINE_COUNT_USE END,0) AS LINE_COUNT_IDLE	 --20150116 고상현 : 임시 저장된 데이터를 보여주지 않기 위해 수정 
     , TOTAL_OVERTIME 
     ...
     /* 2014.03.03 김준혁 입력을 대 단위로 하므로 1/1,000해서 구현 부탁합니다. (소수점 절사하지 말고 끝으로 붙도록…) */	   	 
     , ACTUAL_PRODUCTION_CAPA/1000 AS ACTUAL_PRODUCTION_CAPA
     , '' AS DUMMY1
     , '' AS DUMMY2
     , '' AS DUMMY3
     , '' AS DUMMY4
  ...
  FROM (	
          SELECT MONTH_rn
               ...
               , CASE WHEN OHP.INPUT_YN = 'Y' OR OHP.KPI_PERIOD_CODE = :START_PERIOD THEN  --20150116 고상현 : 임시 저장된 데이터를 보여주지 않기 위해 수정 
                      ...
                      WHEN OHP.INPUT_YN <> 'Y' AND OHP.KPI_PERIOD_CODE = :KPI_PERIOD_STD THEN
               ...
               /* 2014.02.17 WIP_LINE 테이블에 설정된 라인의 개수를 가져옴 */
               , LCT.LINE_COUNT_TOTAL
            ...
            FROM (
                    SELECT MASTER.FACTORY_REGION1
                      ...
                      FROM (
                              SELECT ROWNUMBER() OVER(ORDER BY T.YYYYMM_ORIG_ID ) AS MONTH_rn
                               ...
                               WHERE T.YYYYMM_ORIG_ID BETWEEN :START_PERIOD AND :END_PERIOD
                   ...
                   WHERE KIT.FACTORY_REGION1 = MASTER.FACTORY_REGION1
                     ...
                     AND KIT.KPI_PERIOD_CODE = :KPI_PERIOD_STD
                     AND KIT.DATE_CODE       = 'M'
             ...
             AND F.KPI_PERIOD_STD  = :KPI_PERIOD_STD
             AND F.YMW_CODE        = OHP.DATE_CODE	

                 LEFT OUTER JOIN 
                 (  
                    SELECT FACTORY_REGION1
                         , GBU_CODE
                         , COUNT(*) AS LINE_COUNT_TOTAL 
                      FROM KWL 
                     WHERE CROSS_LINE_YN = 'N'
                     GROUP BY FACTORY_REGION1
                         , GBU_CODE
                 ) AS LCT
              ON LCT.FACTORY_REGION1 = OHP.FACTORY_REGION1
             AND LCT.GBU_CODE        = OHP.GBU_CODE   
                 ...
                 (
                    SELECT FL.TARGET_CODE
                      ...
                      FROM KWL
                       ...
                       AND FL.KPI_PERIOD_STD   = :KPI_PERIOD_STD
                     WHERE 1 = 1
             ...
             AND FL.DATE_CODE       = OHP.DATE_CODE	
                 ...
                 (
                    SELECT L.TARGET_CODE
                         ...
                         , SUM(L.PRODUCTION_QUANTITY) AS PRODUCTION_QUANTITY
                      FROM PDSC.OC_B100_XXLGE_LOOKUP_VALUES LV
                           INNER JOIN PDSC.OC_B100_PD_KPI_OPERATION_RATE_L L
                        ON LV.FACTORY_REGION = L.FACTORY_REGION1
                       AND LV.GBU_CODE       = L.GBU_CODE
                       AND LV.MEANING        = L.LINE_NAME
                     WHERE LV.LOOKUP_TYPE = 'PD_INPUT_TEMPLET_MANUAL_LINE'
                       AND L.DATE_CODE = 'M'                         
                       AND L.KPI_PERIOD_CODE BETWEEN :START_PERIOD AND :END_PERIOD --:KPI_PERIOD_STD
                       AND LV.ATTRIBUTE2 = :KPI_PERIOD_STD
                       --[중요: 조회 기간에 대한 의견]
